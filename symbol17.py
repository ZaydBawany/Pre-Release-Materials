...
class Puzzle():
    ...
    #changes start here
    def RemoveBlockedCell(self):
        Valid = False
        while not Valid:
            try:
                Row = int(input("Enter row number of @ symbol to remove: "))
                Valid = True
            except:
                pass
        Column = -1
        ...
        while not Valid:
            try:
                Column = int(input("Enter column number of @ symbol to remove: "))
                Valid = True
            except:
                pass
        Index = (self.__GridSize - Row) * self.__GridSize + Column - 1
        if  self.__Grid[Index].GetSymbol() == "@":
            self.__Grid[Index] = Cell()  # This replaces the BlockedCell with a regular Cell
            print("Cell is now unblocked")
        else:
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            self.__SymbolsLeft -= 3 #changed for testing purposes
            print(f"You have {self.__SymbolsLeft} symbols left")# changed for testing purposes
            ...
            if self.__SymbolsLeft % 5 == 0:
                self.RemoveBlockedCell()
            elif self.__SymbolsLeft == 0:
                Finished = True
        print()
        self.DisplayPuzzle()
        print()
