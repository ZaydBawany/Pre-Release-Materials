...
#changes start
def CheckSymbolAllowed(self, SymbolToCheck):
    result = False  # Initialize a result variable to use SymbolToCheck
    for Item in self.__SymbolsNotAllowed:
        if Item == SymbolToCheck:
            result = False  # Set the result based on SymbolToCheck
            break
        else:
            result = True  # Set the result to True if SymbolToCheck is not found
    return result
#changes end
