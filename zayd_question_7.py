...
class Puzzle():
    ...
    def __GetSymbolFromUser(self):
        ...
        while not Symbol in self.__AllowedSymbols:
            ...
            t_pattern = "TTT**T**T"
            x_pattern = "X*X*X*X*X"
            Symbol = input(f"Enter symbol to place (Q: {q_pattern}, T: {t_pattern}, X: {x_pattern}) or enter S to shuffle (costs 3 moves): ")
            # If user selects to shuffle the grid
            if Symbol == "S":
                # Remember the positions and symbols of blocked cells
                blocked_positions = [i for i, cell in enumerate(self.__Grid) if isinstance(cell, BlockedCell)]
                blocked_symbols = [cell.GetSymbol() for cell in self.__Grid if isinstance(cell, BlockedCell)]
                
                # Extract symbols from all cells (including non-blocked)
                all_symbols = [cell.GetSymbol() for cell in self.__Grid]
                
                # Shuffle all the symbols
                random.shuffle(all_symbols)
                
                # Re-insert blocked cell symbols into their original positions
                for position, symbol in zip(blocked_positions, blocked_symbols):
                    all_symbols[position] = symbol
                
                # Update the grid with shuffled symbols
                for cell, symbol in zip(self.__Grid, all_symbols):
                    cell.ChangeSymbolInCell(symbol)
                
                # Deducting 3 points from the user's score for using the shuffle feature
                self.__Score -= 3
                
                # Display the shuffled grid
                self.DisplayPuzzle()
                return self.__GetSymbolFromUser()
            #end of changes
        return Symbol
